var logger = require('pomelo2-logger').getLogger('pomelo-admin', 'MasterSocket');
...
MasterSocket.prototype.onClient = function (msg) {
	...
	var type = this.type;
	if (type !== Constants.TYPE_CLIENT) {
		logger.error('invalid message to client, but current connect type', { type });
		return;
	}
...
MasterSocket.prototype.onDisconnect = function () {
	...
	logger.debug('disconnect', { id, type, info });
	if (registered) {
	...
	if (type === Constants.TYPE_CLIENT && registered) {
		logger.info('client user exit', { username });
	}
...
MasterSocket.prototype.repushQosMessage = function (serverId) {
	...
	logger.debug('repush qos message', { qosMsgs });
...
MasterSocket.prototype.onError = function (err) {
	logger.error('server error', { id: this.id, message: err.mesage, stack: err.stack });
	this.onDisconnect();
}
