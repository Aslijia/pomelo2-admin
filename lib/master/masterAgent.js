var logger = require('pomelo2-logger').getLogger('pomelo-admin', 'MasterAgent');
...
MasterAgent.prototype.notifyById = function (serverId, moduleId, msg) {
  ...
  var record = this.idMap[serverId];
  if (!record) {
    logger.error('fail to notifyById for unknown server', { serverId });
    return false;
  }
...
MasterAgent.prototype.notifyByServer = function (serverId, serverInfo, moduleId, msg) {
  ...
  var record = this.idMap[serverId];
  if (!record) {
    logger.error('fail to notifyByServer for unknown server', { serverId });
    return false;
  }
...
MasterAgent.prototype.notifySlavesById = function (serverId, moduleId, msg) {
  ...
  if (!slaves || slaves.length === 0) {
    logger.error('fail to notifySlavesById for unknown server', { serverId });
    return false;
  }
...
MasterAgent.prototype.notifyClient = function (clientId, moduleId, msg) {
  ...
  if (!record) {
    logger.error('fail to notifyClient for unknown client', { clientId });
    return false;
  }
...
MasterAgent.prototype.doAuthUser = function (msg, socket, cb) {
  ...
  authUser(msg, env, function (user) {
    ...
    logger.info('client user login to master', { username });
