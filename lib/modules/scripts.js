 ...
 * MIT Licensed
 */
var monitor = require('pomelo2-monitor');
var logger = require('pomelo2-logger').getLogger('pomelo-admin', __filename);
var monitor = require('pomelo2-monitor');
var vm = require('vm');
var fs = require('fs');
...
var get = function (scriptModule, agent, msg, cb) {
    ...
    fs.readFile(path.join(scriptModule.root, filename), 'utf-8', function (err, data) {
        if (err) {
            logger.error('fail to read script file', { filename, stack: err.stack });
...
var save = function (scriptModule, agent, msg, cb) {
    ...
    fs.writeFile(filepath, msg.body, function (err) {
        if (err) {
            logger.error('fail to write script file', { filename, message: err.message });
...
var run = function (scriptModule, agent, msg, cb) {
    agent.request(msg.serverId, module.exports.moduleId, msg, function (err, res) {
        if (err) {
            logger.error('fail to run script', { message: err.message });
            return;
        }
        cb(null, res);
    });
