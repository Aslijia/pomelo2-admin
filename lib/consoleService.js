var logger = require('pomelo2-logger').getLogger('pomelo-admin', 'ConsoleService');
var MonitorAgent = require('./monitor/monitorAgent');
var EventEmitter = require('events').EventEmitter;
var MasterAgent = require('./master/masterAgent');
var schedule = require('pomelo2-scheduler');
...
ConsoleService.prototype.start = function (cb) {
	if (this.master) {
		...
		logger.info('try to connect master', { type: this.type, host: this.host, port: this.port });
...
ConsoleService.prototype.execute = function (moduleId, method, msg, cb) {
	...
	if (!m) {
		logger.error('unknown module.', { moduleId });
		cb('unknown moduleId:' + moduleId);
		return;
	}

	if (!m.enable) {
		logger.error('module is disable.', { moduleId });
	...
	if (!module || typeof module[method] !== 'function') {
		logger.error('module dose not have a method called.', { moduleId, method });
...
var enableCommand = function (consoleService, moduleId, msg, cb) {
	if (!moduleId) {
		logger.error('fail to enable admin module', { moduleId });
		cb('empty moduleId');
		return;
	}
