var logger = require('pomelo2-logger').getLogger('pomelo-admin', 'MqttClient');
...
MqttClient.prototype.connect = function (host, port, cb) {
	...
	logger.info('try to connect', { host: this.host, port: this.port });
	this.socket.connect({
	...
	this.socket.on('publish', function (pkg) {
		...
		logger.debug('publish message', { topic, msg });
		self.emit(topic, msg);
	});

	this.socket.on('close', function () {
		logger.error('mqtt socket is close, remote server', { host, port });
		self.onSocketClose();
	});

	this.socket.on('error', function (err) {
		logger.error('mqtt socket is error, remote server', { host, port });
	...
	this.socket.on('disconnect', function () {
		logger.error('mqtt socket is disconnect, remote server', { host, port });
...
MqttClient.prototype.addTimeout = function (reconnectFlag) {
	...
	this.timeoutId = setTimeout(function () {
		...
		logger.error('mqtt client connect', { host: self.host, port: self.port, timeout: self.timeout / 1000 });
...
MqttClient.prototype.reconnect = function () {
	...
	logger.debug('MqttClient reconnect', { delay });
	this.reconnectId = setTimeout(function () {
		logger.info('reconnect delay', { delay: delay / 1000 });
	...
	if (this.lastPing > 0) {
		if (this.lastPong < this.lastPing) {
			if (now - this.lastPing > KEEP_ALIVE_TIMEOUT) {
				logger.error('mqtt rpc client checkKeepAlive error timeout', { timeout: KEEP_ALIVE_TIMEOUT });
				this.close();
			}
		} else {
			this.socket.pingreq();
