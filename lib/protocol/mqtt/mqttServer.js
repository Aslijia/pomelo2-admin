var logger = require('pomelo2-logger').getLogger('pomelo-admin', 'MqttServer');
...
MqttServer.prototype.listen = function (port) {
	...
	this.server.on('error', function (err) {
		logger.error('mqtt server is error', { message: err.message });
		self.emit('error', err);
	});
	...
	this.server.on('connection', function (stream) {
		...
		socket.on('publish', function (pkg) {
			...
			logger.debug('MqttServer publish message', { topic, msg });
