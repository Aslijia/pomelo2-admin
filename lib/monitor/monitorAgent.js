var logger = require('pomelo2-logger').getLogger('pomelo-admin', 'MonitorAgent');
...
MonitorAgent.prototype.connect = function (port, host, cb) {
  ...
  this.socket.on('register', function (msg) {
    if (msg && msg.code === protocol.PRO_OK) {
      ...
      self.emit('close');
      logger.error('server register master failed', { id: self.id, type: self.type });
    }
  });
...
MonitorAgent.prototype.notify = function (moduleId, msg) {
  if (this.state !== ST_REGISTERED) {
    logger.error('agent can not notify now, state:', { state: this.state });
    return;
  }
...
MonitorAgent.prototype.request = function (moduleId, msg, cb) {
  if (this.state !== ST_REGISTERED) {
    logger.error('agent can not request now, state', { state: this.state });
    return;
  }
